...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.alerts.periodical.' + index,
    module: InputIO,
    index: index
  },)
})
...
export default {
  ...
  filters: [
    ...
    function (doc, opts, next, pipeline) {
      debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.alerts.periodical').conn_pollers)
      ...
      if (buffer.length < Object.getLength(pipeline.get_input_by_id('input.alerts.periodical').conn_pollers)) { buffer.push(Object.clone(doc)) }

      if (buffer.length >= Object.getLength(pipeline.get_input_by_id('input.alerts.periodical').conn_pollers)) { // || buffer_expire < Date.now()
        _merge()
      }
      // else {
  ...
  output: [
    ...
    //
    //   if (!payload.err) { EventBus.$emit('input.alerts.periodical.' + payload.metadata.input, payload) }
    //
    //   // if (!payload.err) { EventBus.$emit('log', payload) }
    // }
    function (payload) {
      if (!payload.err && /^input\.alerts\.periodical\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.alerts.periodical[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.alerts.periodical.' + payload.metadata.input, payload)
      }
