...
const debug = Debug('apps:system:pipelines:category')
...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.system.category' + index,
    module: InputIO,
    index: index
  },)
})
...
export default {
  ...
  filters: [
    ...
    // function (doc, opts, next, pipeline) {
    //   debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.system.category').conn_pollers)
    //
    //   let timeout
    //
    //   let _merge = function () {
    ...
    //
    //   if (buffer.length === Object.getLength(pipeline.get_input_by_id('input.system.category').conn_pollers)) { // || buffer_expire < Date.now()
    //     _merge()
    //   } else {
    //     buffer.push(doc)
  ...
  output: [
    ...
    //
    //   if (!payload.err) { EventBus.$emit('input.system.category.' + payload.metadata.input, payload) }
    //
    //   // if (!payload.err) { EventBus.$emit('log', payload) }
    // }
    function (payload) {
      if (!payload.err && /^input\.system\.category\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.system.category[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.system.category.' + payload.metadata.input, payload)
      }
