...
const debug = Debug('apps:system:pipelines:host:minute')
...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.system.host.minute' + index,
    module: InputIO,
    index: index
  },)
})
...
export default {
  ...
  filters: [
    ...
    // function (doc, opts, next, pipeline) {
    //   debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.system.host.minute').conn_pollers)
    //
    //   let timeout
    //
    //   let _merge = function () {
    ...
    //
    //   if (buffer.length === pipeline.get_input_by_id('input.system.host.minute').conn_pollers) { // || buffer_expire < Date.now()
    //     _merge()
    //   } else {
    //     buffer.push(doc)
  ...
  output: [
    ...
    //
    //   if (!payload.err) { EventBus.$emit('input.system.host.minute.' + payload.metadata.input, payload) }
    //
    //   // if (!payload.err) { EventBus.$emit('log', payload) }
    // }
    function (payload) {
      if (!payload.err && /^input\.system\.host\.minute\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.system.host.minute[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.system.host.minute.' + payload.metadata.input, payload)
      }
