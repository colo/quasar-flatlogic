...
const debug = Debug('apps:system:pipelines:categories')
...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.system.categories.' + index,
    module: InputIO,
    index: index
  },)
})
...
export default {
  ...
  filters: [
    ...
    function (doc, opts, next, pipeline) {
      debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.system.categories').conn_pollers)
      ...
      if (buffer.length < Object.getLength(pipeline.get_input_by_id('input.system.categories').conn_pollers)) { buffer.push(Object.clone(doc)) }

      // if (buffer.length === pipeline.get_input_by_id('input.system.categories').conn_pollers) { // || buffer_expire < Date.now()
      if (buffer.length >= Object.getLength(pipeline.get_input_by_id('input.system.categories').conn_pollers)) { // || buffer_expire < Date.now()
        _merge()
      }
      // else {
  ...
  output: [
    function (payload) {
      if (!payload.err && /^input\.system\.categories\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.system.categories[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.system.categories.' + payload.metadata.input, payload)
      }
