...
const debug = Debug('apps:system:pipelines:hosts:minute')
...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.system.hosts.minute.' + index,
    module: InputIO,
    index: index
  },)
})
export default {
  ...
  filters: [
    ...
    function (doc, opts, next, pipeline) {
      debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.system.hosts.minute').conn_pollers)
      ...
      if (buffer.length < Object.getLength(pipeline.get_input_by_id('input.system.hosts.minute').conn_pollers)) { buffer.push(Object.clone(doc)) }

      if (buffer.length >= Object.getLength(pipeline.get_input_by_id('input.system.hosts.minute').conn_pollers)) { // || buffer_expire < Date.now()
        _merge()
      }
      // else {
  ...
  output: [
    function (payload) {
      if (!payload.err && /^input\.system\.hosts\.minute\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.system.hosts.minute[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.system.hosts.minute.' + payload.metadata.input, payload)
      }
