...
const debug = Debug('apps:system:pipelines:hosts:periodical')
...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.system.hosts.periodical.' + index,
    module: InputIO,
    index: index
  },)
})
export default {
  input: [
    {
      poll: {
        suspended: true,
        id: 'input.system.hosts.periodical',
        conn: ios,
  ...
  filters: [
    ...
    function (doc, opts, next, pipeline) {
      debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.system.hosts.periodical').conn_pollers)
      ...
      if (buffer.length < Object.getLength(pipeline.get_input_by_id('input.system.hosts.periodical').conn_pollers)) { buffer.push(Object.clone(doc)) }

      if (buffer.length >= Object.getLength(pipeline.get_input_by_id('input.system.hosts.periodical').conn_pollers)) { // || buffer_expire < Date.now()
        _merge()
      }
      // else {
  ...
  output: [
    function (payload) {
      if (!payload.err && /^input\.system\.hosts\.periodical\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.system.hosts.periodical[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.system.hosts.periodical.' + payload.metadata.input, payload)
      }
