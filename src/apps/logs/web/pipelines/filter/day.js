...
const debug = Debug('apps:logs:web:pipelines:filter:day')
...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.logs.web.filter.day.' + index,
    module: InputIO,
    index: index
  },)
})
...
export default {
  ...
  filters: [
    ...
    function (doc, opts, next, pipeline) {
      debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.logs.web.filter.day').conn_pollers)
      ...
      if (buffer.length < Object.getLength(pipeline.get_input_by_id('input.logs.web.filter.day').conn_pollers)) { buffer.push(Object.clone(doc)) }

      if (buffer.length >= Object.getLength(pipeline.get_input_by_id('input.logs.web.filter.day').conn_pollers)) { // || buffer_expire < Date.now()
        _merge()
      }
      // else {
  ...
  output: [
    ...
    //
    //   if (!payload.err) { EventBus.$emit('input.logs.web.filter.day.' + payload.metadata.input, payload) }
    //
    //   // if (!payload.err) { EventBus.$emit('log', payload) }
    // }
    function (payload) {
      if (!payload.err && /^input\.logs\.web\.filter\.day\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.logs.web.filter.day[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.logs.web.filter.day.' + payload.metadata.input, payload)
      }
